...
public class BoardServiceTest {
	...
	@MockBean  // BoardService 타입의 구현 객체를 모킹(and 목업)해라.
	...
	public void testGetBoard() throws Exception {
		...
		board.setWriter("No name");
		board.setContent("No contents");
		...
		.andExpect(status().is(200))
		.andExpect(jsonPath("$.writer").value("No name"));
	}
	...
	public void testGetBoardList() throws Exception {
		...
		for (int i = 1; i <= 5; i++) {
			...
			board.setWriter("name" + i);
			board.setContent("contents" + i);
			board.setRegDate(new Date());
			board.setCnt(0);
			boardList.add(board);
		}				
		...
		.andExpect(status().is(200))
		.andExpect(jsonPath("$[0].writer").value("name1"))
